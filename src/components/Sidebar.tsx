...
export function Sidebar({ isCollapsed, onToggle }: SidebarProps) {
  ...
  const handleRemoveUser = async (usernameToRemove: string) => {
    const confirmDelete = await confirmAction({
      ...
      text: `Are you sure you want to delete the portal credential for "${usernameToRemove}"?`,
      icon: "warning",
        ...
        <div className="p-6 border-b border-sidebar-border/20 bg-sidebar-accent/10">
          <div className="flex items-center justify-between gap-3">
            ...
            {onToggle && (
              <button
                onClick={onToggle}
                className="p-1.5 rounded-lg text-sidebar-foreground hover:bg-white/10 transition-colors"
                title="Hide Sidebar"
              >
